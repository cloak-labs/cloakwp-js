...
const debugEnabled = typeof process !== "undefined" &&
...
const editorViewportListeners = new Set();
/**
...
export const applyPreviewViewportTokens = (heightPx) => {
...
export const sendBlockHeightToWP = (h = null, context) => {
 ...
 */
const getOverflowingContentBottom = (root) => {
...
export function getConfiguredWpOrigin() {
    try {
...
export function isTrustedWpOrigin(origin) {
    try {
        ...
        if (TRUSTED_WP_HOSTS.has(host))
            return true;
        return TRUSTED_WP_HOST_SUFFIXES.some((suffix) => host.endsWith(suffix) && host.length > suffix.length);
    }
    catch {
        return false;
    }
 ...
 */
export function resolvePreviewTargetOrigin(preferred) {
    if (preferred) {
        try {
    ...
    if (typeof window !== "undefined") {
        ...
        if (ancestors && ancestors.length > 0) {
            ...
            if (isTrustedWpOrigin(topOrigin))
                return topOrigin;
        }
        if (document.referrer) {
            try {
                const referrerOrigin = new URL(document.referrer).origin;
                if (isTrustedWpOrigin(referrerOrigin))
...
function postMessageToWpEditor(payload, targetOrigin) {
...
function isMessageFromWpEditor(event, targetOrigin) {
    const fromEmbedder = event.source === window.parent ||
        (window.top != null && event.source === window.top);
    return fromEmbedder && event.origin === targetOrigin;
}
export function sendPreviewReadyToWp(previewKey, targetOrigin = resolvePreviewTargetOrigin()) {
    if (!previewKey || !targetOrigin)
        return;
    postMessageToWpEditor({
...
export function handleWPBlockIframeMessage(event, { previewKey, targetOrigin = resolvePreviewTargetOrigin(), onBlockDataReceipt, }) {
    ...
    if (payload.blockData && typeof payload.blockData === "object") {
        ...
        onBlockDataReceipt?.(payload.blockData);
    }
    ...
    if (typeof payload.previewViewportHeight === "number" &&
...
export const watchForDocumentHeightChanges = (options) => {
    ...
    const valveTimestamps = [];
    let debounceTimer = null;
    let debouncedHeight = null;
    /**
     ...
     */
    const tryViewportBootstrap = () => {
        if (!bootstrapPending || editorViewportHeight <= 0)
            return false;
        ...
        const iframeAlreadyTall = viewportHeight >= editorViewportHeight - BOOTSTRAP_GAP_PX;
    ...
    const report = (height, via) => {
        const h = Math.round(height);
        if (!Number.isFinite(h) || h < 20 || h > MAX_TARGET_PX)
            return;
        if (lastSentHeight != null &&
    ...
    const reportDebounced = (height, via) => {
        debouncedHeight = height;
        if (debounceTimer)
            clearTimeout(debounceTimer);
        debounceTimer = setTimeout(() => {
            debounceTimer = null;
            if (debouncedHeight != null)
                report(debouncedHeight, via);
            debouncedHeight = null;
        }, DEBOUNCE_MS);
    };
    const valveOpen = () => {
    ...
    if (typeof document !== "undefined") {
        ...
        const onAsyncLayout = () => {
            if (disconnected || asyncLayoutRaf)
                return;
        ...
        observer.disconnect = () => {
            ...
            if (asyncLayoutRaf)
                cancelAnimationFrame(asyncLayoutRaf);
