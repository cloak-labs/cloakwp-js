...
export function parseRevalidationBody(rawBody) {
    let body;
    try {
    ...
    for (const value of paths) {
        ...
        if (!normalized) {
            invalidResults.push({
                ...
                status: "invalid",
                error: "Path must be a canonical absolute pathname without a query or hash.",
            });
            continue;
        }
...
export async function verifyRevalidationRequest(request, rawBody, secret, now = Date.now()) {
    if (!secret) {
        return false;
    }
    ...
    const expectedSignature = bytesToHex(await hmacSha256(secret, `${timestamp}.${rawBody}`));
    return constantTimeEqual(suppliedSignature.toLowerCase(), expectedSignature);
}
export async function handleRevalidationRequest(request, options) {
    if (request.method !== "POST") {
        return json({ error: "Method not allowed." }, 405);
    }
    const rawBody = await request.text();
    if (!(await verifyRevalidationRequest(request, rawBody, options.secret, options.now))) {
    ...
    const results = [...parsed.invalidResults];
    const warmedPaths = [];
    ...
    const status = hasError || (hasInvalid && warmedPaths.length > 0)
        ? 207
        : hasInvalid
            ? 400
            : 200;
